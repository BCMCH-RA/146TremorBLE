...
// resetCSV() (T4 below) so "Refresh buffer" and "Start recording" (which
// already calls resetCSV() internally) both begin a clean tremor session,
// matching the existing one-command semantics rather than adding a
// separate reset path.
...
void resetCSV() {
    ...
    csvRowCount = 0;
    xSemaphoreGive(imuMutex);
    tremorSeverityReset();   // T5: "Refresh buffer" / "Start recording" (which
                              // already calls resetCSV()) both begin a clean
                              // tremor session, same one-command semantics
